# OpenTT Pravila Korišćenja Imena i Brenda

## 1. Svrha

OpenTT je slobodan i otvoren softver namenjen digitalizaciji i dugoročnom očuvanju infrastrukture stonog tenisa.
Izvorni kod je licenciran pod GNU AGPL uslovima, koji daju široka prava korišćenja, proučavanja, izmena i deljenja softvera.

Ovaj dokument reguliše posebnu temu: korišćenje OpenTT naziva, logotipa i identiteta projekta.
...
- naziv `OpenTT`,
- OpenTT logotipe i vizuelne oznake,
...
- korišćenje OpenTT koda,
- proučavanje i verifikacija OpenTT koda,
- izmena OpenTT koda,
- pokretanje sopstvenih OpenTT instanci,
- forkovanje i redistribucija uz poštovanje AGPL obaveza.

Ako nudite modifikovani OpenTT kao mrežni servis, primenjuju se AGPL obaveze deljenja izvornog koda.

## 4. Pravila brenda i identiteta

Čak i kada je upotreba koda dozvoljena, OpenTT brend se ne sme koristiti na način koji može dovesti korisnike u zabludu.

### 4.1 Dozvoljeno bez prethodne dozvole

- tačno navođenje OpenTT-a kao izvornog (upstream) projekta,
- činjenične izjave tipa "zasnovano na OpenTT" ili "fork OpenTT-a",
- linkovanje ka zvaničnom repozitorijumu i dokumentaciji,
- javna diskusija kompatibilnosti sa OpenTT-om,
- doprinosi zajednici uz tačno navođenje izvora.

### 4.2 Nije dozvoljeno bez prethodne dozvole

- predstavljanje nezvanične verzije/forka kao zvaničnog OpenTT projekta,
- korišćenje OpenTT imena/logotipa na način koji zbunjuje korisnike o autorstvu ili odobrenju,
...
- korišćenje OpenTT brenda kao primarnog identiteta divergentnog forka bez jasnog razlikovanja.

## 5. Obavezna transparentnost za forkove i derivate

Ako objavljujete fork ili derivatnu distribuciju, preporučeno je da:
- jasno navedete da je nezvanična verzija (npr. "Nezvanični fork OpenTT-a"),
- vidljivo uputite na upstream OpenTT projekat,
- izbegnete naziv/branding koji implicira da je vaša verzija kanonski upstream.

Preporučene formulacije:
- "Ovaj projekat je nezavisni fork zasnovan na OpenTT-u."
- "Nije zvanično povezan sa OpenTT projektom niti od njega odobren."

## 6. Smernice za domene, društvene naloge i distribuciju

Radi smanjenja konfuzije korisnika, izbegavati:
- domene koji deluju kao zvanična OpenTT infrastruktura, osim ako ste zvanični maintainer,
- social handle-ove ili package nazive koji sugerišu kanonsko vlasništvo.

Preporuka je eksplicitan naziv derivata, na primer:
- `<vas-org>-opentt`,
- `opentt-<region>-fork`,
- `<ime-proizvoda> (zasnovano na OpenTT)`. 

## 7. Upotreba logotipa i vizuelnog identiteta

Zvanični OpenTT logotipi/oznake ne treba da budu primarni identitet modifikovane distribucije bez dozvole.
...
Samo zvanični kanali projekta mogu objavljivati poruke kao "OpenTT official".
...
- da li je u pitanju zvanični OpenTT ili derivat treće strane.
...
Takve module ne treba predstavljati kao finalizovane zvanične OpenTT podsisteme, osim ako maintainers to eksplicitno objave.

Pri distribuciji takvih modula, potrebno je jasno komunicirati nivo zrelosti i release status.

## 11. Traženje dozvole

Ako želite da koristite OpenTT ime/logo na način koji nije jasno obuhvaćen ovim pravilima, tražite dozvolu maintainera pre objavljivanja.
...
OpenTT kod je otvoren i forkabilan pod AGPL uslovima.
OpenTT identitet (naziv/logo/zvanično predstavljanje) ne sme se koristiti na obmanjujući način.
