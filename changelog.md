...
- Extracted league-admin UI rendering into `src/WordPress/UserPortalLeagueAdminViewService.php` (`renderLeagueAdminTools`) and switched `UserPortalManager` to dependency-based delegation for league/season tabs, match cards, and inline games forms; `UserPortalManager` reduced further from ~1271 to ~1078 LOC with no intended behavior change.
...
- Reworked frontend league match game-entry flow (`UserPortalLeagueGamesService`) to support lineup-based generation: added A/B/C vs Y/X/Z lineup selectors with reserves, auto-population of singles by league match format and result-limited game count, and intentional blank doubles rows for manual player entry; each generated game remains fully editable before save.
- Extended frontend save pipeline (`UserPortalLeagueMatchService` + `UserPortalManager` delegation) to pass submitted lineup context into batch game application so missing singles players can be auto-resolved server-side from selected lineup/reserves.
