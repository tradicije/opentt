...
require_once __DIR__ . '/class-opentt-unified-competition-presentation-service.php';
...
require_once __DIR__ . '/class-opentt-unified-grid-render-service.php';
require_once __DIR__ . '/class-opentt-unified-shortcode-ui-service.php';
...
final class OpenTT_Unified_Core
{
    ...
    public static function handle_frontend_search_ajax()
    {
        ...
        $context = [];
        if (isset($_POST['context'])) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
            $raw = wp_unslash($_POST['context']); // phpcs:ignore WordPress.Security.NonceVerification.Missing
            if (is_string($raw)) {
        ...
        if ($query === '') {
            ...
            wp_send_json_success([
                'query' => '',
                'groups' => $groups,
                'querySuggestions' => self::search_query_suggestions($context),
            ]);
        }

        $groups = self::build_search_groups($query, $limit, $context);
        $intent = self::search_parse_intent($query, $limit, $context);
...
HTML;
    ...
    public static function render_match_edit_page()
    {
        ...
        if ($match) {
            ...
            echo '<style>
                .opentt-admin-games-ui{display:flex;flex-direction:column;gap:12px}
                .opentt-admin-teams-overview{border:1px solid #cbd5e1;border-radius:12px;background:#f8fafc;box-shadow:0 1px 2px rgba(15,23,42,.04)}
                .opentt-admin-teams-overview .meta{padding:10px 12px 4px;text-align:center;font-size:12px;color:#475569;font-weight:600}
                .opentt-admin-teams-overview .row{display:grid;grid-template-columns:1fr auto 1fr;gap:10px;align-items:center;padding:8px 12px}
                .opentt-admin-teams-overview .team{display:flex;align-items:center;gap:10px;min-width:0}
                .opentt-admin-teams-overview .team.away{justify-content:flex-end}
                .opentt-admin-teams-overview .logo{width:42px;height:42px;border-radius:8px;border:1px solid #cbd5e1;background:#fff;object-fit:cover;flex:0 0 auto}
                .opentt-admin-teams-overview .name{font-weight:700;color:#0f172a;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
                .opentt-admin-teams-overview .score{padding:6px 12px;border:1px solid #94a3b8;border-radius:10px;background:#fff;font-weight:800;font-size:20px;line-height:1;color:#1e293b}
                .opentt-admin-teams-overview .footer{padding:2px 12px 10px;text-align:center;font-size:12px;color:#64748b}
                .opentt-admin-lineup{border:1px solid #e2e8f0;border-radius:12px;background:#fff;padding:12px}
                .opentt-admin-lineup-head{margin:0 0 10px 0}
                .opentt-admin-lineup-grid{display:grid;grid-template-columns:1fr 1fr;gap:14px}
                .opentt-admin-lineup-col{display:flex;flex-direction:column;gap:8px}
                .opentt-admin-lineup-col h4{margin:0 0 2px 0}
            </style>';
            echo '<div class="opentt-admin-games-ui">';
            echo '<section class="opentt-admin-teams-overview">';
            ...
            echo '<section class="opentt-admin-lineup">';
            echo '<div class="opentt-admin-lineup-head">';
            echo '<h3 style="margin:0;">Sastavi za auto-popunu partija</h3>';
            echo '<p class="description" style="margin:6px 0 0;">Levo domaći (A/B/C + više rezervi), desno gosti (Y/X/Z + više rezervi).</p>';
            echo '</div>';
            echo '<div class="opentt-admin-lineup-grid">';
            echo '<fieldset class="opentt-admin-lineup-col">';
            echo '<h4>Domaćin</h4>';
            echo '<label>A';
            ...
            echo '</fieldset>';
            echo '</div>';
            echo '<p style="margin:10px 0 0;"><button type="button" class="button" id="opentt-apply-lineups">Primeni sastave na partije</button></p>';
            echo '</section>';
    ...
    public static function render_club_edit_page()
    {
        ...
        echo '</div>';
        echo '</form></div>';
        echo '<style>.opentt-cover-focus-wrap{margin-top:10px}.opentt-cover-focus-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.opentt-cover-focus-preview{margin-top:6px;border:1px solid #ccd0d4;background:#111;overflow:hidden}.opentt-cover-focus-preview.is-desktop{aspect-ratio:16/5}.opentt-cover-focus-preview.is-mobile{aspect-ratio:16/5}.opentt-cover-focus-preview img{width:100%;height:100%;display:block;object-fit:cover}.opentt-cover-focus-controls{margin-top:10px;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.opentt-cover-focus-controls label{display:block;font-weight:600}.opentt-cover-focus-controls input[type=range]{width:100%}@media (max-width:782px){.opentt-cover-focus-grid,.opentt-cover-focus-controls{grid-template-columns:1fr}}</style>';
        echo <<<'JS'
<script>
(function($){
    var frame;
...
JS;
        ...
        return [
            ...
            [
                ...
                'builder' => [
                    ...
                    ['name' => 'min_chars', 'label' => 'Min karaktera', 'type' => 'number', 'default' => '1', 'help' => 'Koliko karaktera mora da se unese pre prve pretrage.'],
            ...
            [
                ...
                'details' => 'Prikazuje jedno kolo odjednom i prebacuje između kola bez reload-a. `highlight` prima ID, slug ili naziv kluba i naglašava redove utakmica tog kluba. Podržano je i `highlight="auto"` na single-klub stranici.',
                'builder' => [
                    ['name' => 'liga', 'label' => 'Liga slug', 'type' => 'text', 'default' => '', 'help' => 'Slug lige/takmičenja.'],
                    ['name' => 'season', 'label' => 'Season slug', 'type' => 'text', 'default' => '', 'help' => 'Slug sezone (npr. 2025-26).'],
                    ['name' => 'played', 'label' => 'Played', 'type' => 'text', 'default' => '', 'help' => 'true = odigrane, false = neodigrane, prazno = sve.'],
                    ['name' => 'kolo', 'label' => 'Kolo slug', 'type' => 'text', 'default' => '', 'help' => 'Opcioni početni round slug.'],
                    ['name' => 'highlight', 'label' => 'Highlight klub', 'type' => 'text', 'default' => '', 'help' => 'ID, slug ili naziv kluba za naglašavanje redova utakmica; ili `auto` za single-klub kontekst.'],
                    ['name' => 'author', 'label' => 'Author footer', 'type' => 'text', 'default' => 'true', 'help' => 'Prikaz footer bloka `Podatke uneo` (`true`/`false`).'],
                ],
            ],
            [
            ...
            ['tag' => 'opentt_match_teams_short', 'desc' => 'Kompaktni 50/50 teams blok sa centralnim overlay rezultatom.', 'attrs' => 'kontekstualno, title', 'details' => 'Leva i desna polovina koriste boje dresova domaćina/gosta, sa fiksnim centralnim score overlay okvirom.', 'builder' => [['name' => 'title', 'label' => 'Naslov sekcije', 'type' => 'text', 'default' => '']]],
            ...
            ['tag' => 'opentt_club_card', 'desc' => 'Kompaktna kartica kluba (logo, naziv, grad) sa kontekstualnim prikazom.', 'attrs' => 'id, klub, title', 'details' => 'Ako `id/klub` nije prosleđen, na single-klub stranici automatski koristi taj klub.', 'builder' => [['name' => 'id', 'label' => 'ID kluba', 'type' => 'number', 'default' => ''], ['name' => 'klub', 'label' => 'Klub slug/naziv', 'type' => 'text', 'default' => ''], ['name' => 'title', 'label' => 'Naslov sekcije', 'type' => 'text', 'default' => '']]],
            ['tag' => 'opentt_featured_player', 'desc' => 'Istaknuta kartica najboljeg igrača (kontekstualno ili ručni izbor).', 'attrs' => 'igrac, klub, liga, sezona, season, title', 'details' => 'Ako je prosleđen `igrac`, prikazuje tačno tog igrača. Ako je prosleđen `klub` (ili single-klub kontekst), prikazuje najbolje rangiranog igrača tog kluba sa aktuelne rang liste. Ako su dati samo `liga+sezona`, prikazuje top igrača za taj scope i van single-klub stranice.', 'builder' => [['name' => 'igrac', 'label' => 'Igrač slug/ID', 'type' => 'text', 'default' => ''], ['name' => 'klub', 'label' => 'Klub slug/ID', 'type' => 'text', 'default' => ''], ['name' => 'liga', 'label' => 'Liga slug', 'type' => 'text', 'default' => ''], ['name' => 'sezona', 'label' => 'Sezona slug', 'type' => 'text', 'default' => ''], ['name' => 'season', 'label' => 'Season slug', 'type' => 'text', 'default' => ''], ['name' => 'title', 'label' => 'Naslov sekcije', 'type' => 'text', 'default' => '']]],
            ['tag' => 'opentt_club_featured', 'desc' => 'Cover kartica kluba sa posebnom custom slikom (nije WordPress featured image).', 'attrs' => 'klub, id, height, link', 'details' => 'Prikazuje custom club cover koji se podešava u admin panelu ili kroz frontend menadžer tima. Bez `height` koristi prirodan odnos stranica slike; sa `height` radi centralni crop bez stretch-a.', 'builder' => [['name' => 'klub', 'label' => 'Klub slug/naziv', 'type' => 'text', 'default' => ''], ['name' => 'id', 'label' => 'ID kluba', 'type' => 'number', 'default' => ''], ['name' => 'height', 'label' => 'Visina (px)', 'type' => 'number', 'default' => ''], ['name' => 'link', 'label' => 'Link ka klubu', 'type' => 'text', 'default' => 'false']]],
            ['tag' => 'opentt_global_stats', 'desc' => 'Globalni brojači: lige, klubovi, igrači i utakmice.', 'attrs' => 'sezona/season (opciono)', 'details' => 'Bez atributa prikazuje ukupne brojeve. Sa `sezona`/`season` prikazuje podatke samo za tu sezonu (npr. 2025-26).', 'builder' => [['name' => 'sezona', 'label' => 'Sezona slug', 'type' => 'text', 'default' => '', 'help' => 'Ako je prazno, prikaz je ukupno za sve sezone.']]],
    ...
    private static function shortcode_attribute_help_map()
    {
        return [
            ...
            'highlight' => 'ID, slug ili naziv kluba za vizuelno naglašavanje relevantnih redova/stavki; `auto` koristi kontekst single-klub stranice.',
            ...
            'highlight' => 'Naglašava prosleđene klubove u tabeli/listi; podržava i `auto` na single-klub kontekstu.',
    ...
    private static function shortcode_css_class_reference()
    {
        return [
            ...
            'opentt_match_teams_short' => ['module' => 'match-teams-short.css', 'classes' => ['.opentt-match-teams-short', '.opentt-match-teams-short-team', '.opentt-match-teams-short-score-wrap', '.opentt-match-teams-short-score']],
            ...
            'opentt_club_card' => ['module' => 'club-card.css', 'classes' => ['.opentt-club-card', '.opentt-club-card-link', '.opentt-club-card-logo', '.opentt-club-card-name', '.opentt-club-card-city']],
            'opentt_featured_player' => ['module' => 'featured-player.css', 'classes' => ['.opentt-featured-player-card', '.opentt-featured-player-photo', '.opentt-featured-player-name', '.opentt-featured-player-stats']],
            'opentt_club_featured' => ['module' => 'club-featured.css', 'classes' => ['.opentt-club-featured-wrap', '.opentt-club-featured-media', '.opentt-club-featured-image', '.opentt-club-featured-empty']],
            'opentt_global_stats' => ['module' => 'global-stats.css', 'classes' => ['.opentt-global-stats', '.opentt-global-stats-grid', '.opentt-global-stats-card', '.opentt-global-stats-value', '.opentt-global-stats-label']],
...
HTML;
    ...
    public static function render_customize_page()
    {
        ...
        echo '<label><span>Boja zaglavlja tabela</span><small>Menja nijansu pozadine u `thead` zaglavljima na frontend tabelama.</small><input type="text" name="visual_settings[table_head_bg]" value="' . esc_attr((string) ($visual['table_head_bg'] ?? '#081e52')) . '" class="opentt-color-field"></label>';
...
<?php
    ...
    private static function search_parse_intent($query, $limit, array $context)
    {
        ...
        if (preg_match('/^(.+?)\s+poslednjih\s+(\d{1,2})$/u', $folded, $m)) {
            ...
            $club_id = self::search_resolve_club_id_by_phrase($club_phrase, $context);
            if ($club_id > 0) {
                $scope = self::search_resolve_intent_scope_for_club($club_id, $context);
                $position = self::search_compute_club_position($club_id, $scope['liga_slug'], $scope['sezona_slug']);
                $matches = self::search_fetch_recent_club_matches($club_id, $requested_limit, $scope['liga_slug'], $scope['sezona_slug']);
                return [
                    'type' => 'club_recent',
        ...
        if (preg_match('/^(.+?)\s+(sledece|sledecih|naredne|narednih)\s+(\d{1,2})$/u', $folded, $m)) {
            ...
            if ($club_id > 0) {
                $scope = self::search_resolve_intent_scope_for_club($club_id, $context);
                $position = self::search_compute_club_position($club_id, $scope['liga_slug'], $scope['sezona_slug']);
                $matches = self::search_fetch_upcoming_club_matches($club_id, $requested_limit, $scope['liga_slug'], $scope['sezona_slug']);
                return [
                    'type' => 'club_next',
        ...
        if (preg_match('/^koji je (.+) na tabeli$/u', $folded, $m)) {
            ...
            if ($club_id > 0) {
                $scope = self::search_resolve_intent_scope_for_club($club_id, $context);
                $position = self::search_compute_club_position($club_id, $scope['liga_slug'], $scope['sezona_slug']);
                $window = self::search_compute_standings_window($club_id, $scope['liga_slug'], $scope['sezona_slug']);
                return [
                    'type' => 'club_position',
        ...
        $competition = self::search_resolve_competition_from_query($folded, $context);
        if (!empty($competition)) {
            return [
                'type' => 'league_season',
                'label' => 'Liga i sezona',
    ...
    private static function search_text_score($text, $query)
    {
        ...
        $len_q = function_exists('mb_strlen') ? mb_strlen($query_l, 'UTF-8') : strlen($query_l);
        $len_t = function_exists('mb_strlen') ? mb_strlen($text_l, 'UTF-8') : strlen($text_l);
        if ($len_q > 0 && $len_t > 0) {
