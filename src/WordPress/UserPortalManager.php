...
final class UserPortalManager
{
    ...
    private static function renderLeagueAdminTools($userId)
    {
        ...
        $managedPairs = self::getUserManagedLeagueSeasons($userId);
        ...
        foreach ($seasonLeagueMap as $seasonSlug => $leagues) {
            ...
            foreach ($leagues as $leagueIdx => $leagueSlug) {
                ...
                $out .= '<button type="button" class="opentt-league-tab-btn opentt-league-subtab-btn' . esc_attr($leagueActive) . '" data-league-tab="' . esc_attr($seasonSlug . '|' . $leagueSlug) . '">' . esc_html(self::slugToTitle($leagueSlug)) . '</button>';
            }
            $out .= '</div>';
    ...
    private static function renderTeamManagerTools($userId)
    {
        ...
        $out .= '<label>Cover slika kluba <small>(preporuka: 2400x750 px za desktop banner, bitan sadržaj držati u centru zbog mobilnog crop-a)</small></label>';
