...
final class GamesListShortcode
{
    ...
    private static function renderFrontendSubmissionForm($matchRow, array $deps)
    {
        ...
        for ($orderNo = 1; $orderNo <= $maxGames; $orderNo++) {
            ...
            echo '<label><span>Domaći setovi</span><input type="number" min="0" max="7" name="games[' . intval($orderNo) . '][home_sets]" value="" required data-role="home-sets"></label>';
            echo '<label><span>Gost setovi</span><input type="number" min="0" max="7" name="games[' . intval($orderNo) . '][away_sets]" value="" required data-role="away-sets"></label>';
            if ($isDoubles) {
            ...
            for ($setNo = 1; $setNo <= 5; $setNo++) {
                echo '<label data-set-row="' . intval($setNo) . '"><span>Set ' . intval($setNo) . ' (D:G)</span>';
                echo '<span class="opentt-games-submit-set-pair">';
                echo '<input type="number" min="0" max="30" name="games[' . intval($orderNo) . '][sets][' . intval($setNo) . '][home_points]" value="" placeholder="11" data-set-points="home">';
                echo '<span>:</span>';
                echo '<input type="number" min="0" max="30" name="games[' . intval($orderNo) . '][sets][' . intval($setNo) . '][away_points]" value="" placeholder="9" data-set-points="away">';
                echo '</span>';
                echo '</label>';
            }
            echo '</div>';
        ...
        echo '</form>';
        echo '<script>(function(){var forms=document.querySelectorAll(".opentt-games-submit-form");forms.forEach(function(form){if(form.dataset.tabsBound==="1"){return;}form.dataset.tabsBound="1";var tabs=form.querySelectorAll(".opentt-games-submit-tab[data-entry-mode]");var advancedBlocks=form.querySelectorAll("[data-advanced-only=\'1\']");var quickNote=form.querySelector(".opentt-games-submit-mode-note-quick");var advancedNote=form.querySelector(".opentt-games-submit-mode-note-advanced");var modeField=form.querySelector("input[name=\'entry_mode\']");function perGameSetSync(){var games=form.querySelectorAll(".opentt-games-submit-game");games.forEach(function(game){var homeInput=game.querySelector("input[data-role=\'home-sets\']");var awayInput=game.querySelector("input[data-role=\'away-sets\']");var setsWrap=game.querySelector(".opentt-games-submit-sets[data-advanced-only=\'1\']");if(!homeInput||!awayInput||!setsWrap){return;}var homeVal=parseInt(homeInput.value||"0",10);var awayVal=parseInt(awayInput.value||"0",10);if(isNaN(homeVal)){homeVal=0;}if(isNaN(awayVal)){awayVal=0;}var expected=homeVal+awayVal;if(expected<0){expected=0;}if(expected>5){expected=5;}var rows=setsWrap.querySelectorAll("[data-set-row]");rows.forEach(function(row){var no=parseInt(row.getAttribute("data-set-row")||"0",10);var show=(expected>0&&no<=expected&&form.getAttribute("data-entry-mode")==="advanced");row.hidden=!show;var inputs=row.querySelectorAll("input[data-set-points]");inputs.forEach(function(inp){inp.disabled=!show;if(show){inp.required=true;}else{inp.required=false;inp.value="";}});});});}function applyMode(mode){var isAdvanced=mode==="advanced";form.setAttribute("data-entry-mode",isAdvanced?"advanced":"quick");if(modeField){modeField.value=isAdvanced?"advanced":"quick";}tabs.forEach(function(tab){var active=tab.getAttribute("data-entry-mode")===(isAdvanced?"advanced":"quick");tab.classList.toggle("is-active",active);tab.setAttribute("aria-selected",active?"true":"false");});if(quickNote){quickNote.hidden=isAdvanced;}if(advancedNote){advancedNote.hidden=!isAdvanced;}advancedBlocks.forEach(function(block){block.hidden=!isAdvanced;var inputs=block.querySelectorAll("input, select, textarea");inputs.forEach(function(inp){if(!isAdvanced){inp.disabled=true;inp.required=false;}else{inp.disabled=false;}});});perGameSetSync();}tabs.forEach(function(tab){tab.addEventListener("click",function(){applyMode(String(tab.getAttribute("data-entry-mode")||"quick"));});});form.querySelectorAll("input[data-role=\'home-sets\'], input[data-role=\'away-sets\']").forEach(function(inp){inp.addEventListener("input",perGameSetSync);inp.addEventListener("change",perGameSetSync);});applyMode("quick");});})();</script>';
        if ($turnstileEnabled && $turnstileSiteKey !== '') {
